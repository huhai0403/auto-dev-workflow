...
相对原版 BMAD 的差异：双工作流模式（`planning` 全流程 / `pipeline` 增量批处理）、**v0.3.0 新增 `chain_to_pipeline` 默认开启**（一次工具调用跑完 planning + pipeline）、AI 代码审查走 host 端 `/bmad-code-review` skill（不内置 LLM 调用）、原生 MCP 协议、状态持久化与断点续传。
...
| 选项 | 适用场景 | 关键参数 |
|------|---------|---------|
| **1. Planning（新建项目）** | 从 0 开始一个新项目 | `workflow_type=planning`, `output_dir=.bmad-output`, `requirement_description=…` |
...
用户选定一个选项后，模型再调 `start_bmad_workflow`。比如选 1，对应参数：
...
| `requirement_description` | planning 必填 | | 需求描述（pipeline 模式可选） |
...
| `chain_to_pipeline` | | `true` **(v0.3.0 改默认值)** | planning 跑完后**在同一次工具调用内**自动跑 pipeline（按 `planning-artifacts/` 推断 batch）。`false` 退回 v0.2.x 行为 |
...
A: 完整汇总在 `.bmad-output/chain-summary-<workflow_id>.md`，响应消息底部会显示路径。`structuredContent` 里的 `chainPhases` 数组也只包含每阶段的 workflowType/status/completedSteps，不含审计细节。
